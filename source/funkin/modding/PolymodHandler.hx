...
class PolymodHandler
{
  ...
  public static final ALLOWED_MOD_NAME:String = 'Sunndy Mix';
  ...
  public static function loadModsByDir(dirs:Array<String>):Void
  {
    ...
    final DEBUG_ASSET_TYPES:Array<{type:PolymodAssetType, label:String}> = [
      {type: PolymodAssetType.IMAGE, label: 'images'},
      {type: PolymodAssetType.TEXT, label: 'text files'},
      {type: PolymodAssetType.AUDIO_MUSIC, label: 'music files'},
      {type: PolymodAssetType.AUDIO_SOUND, label: 'sound files'},
      {type: PolymodAssetType.AUDIO_GENERIC, label: 'generic audio files'}
    ];

    for (entry in DEBUG_ASSET_TYPES)
    {
      Polymod.listModFiles(entry.type);
    }
    #end
  }
  ...
  public static function getAllMods():Array<ModMetadata>
  {
    ...
    return modMetadata.filter((mod) -> mod.title == ALLOWED_MOD_NAME);
  }
